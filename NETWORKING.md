# Networking — How the Local iMac Talks to Telegram

This document explains how a bot running on a **home iMac** can read Telegram messages and reply, even though the machine sits behind a router with a private IP address.

The key idea: **your iMac never becomes a public server**. It makes **outbound** HTTPS calls to Telegram's cloud, pulls new messages, runs AI locally, and sends replies back out. No port forwarding required.
...
Telegram Cloud (their servers worldwide)
        ▲
        │  HTTPS (Bot API) — outbound from your iMac
        │
   Home router (NAT)
        │
        ▼
...
2. **iMac ↔ Telegram** — the bot's control channel (HTTPS long polling).
3. **iMac ↔ Ollama** — entirely on the machine (loopback, no internet).
...
Your iMac is **not involved yet**.

---

### Step 2: Your iMac asks Telegram "anything new?" (long polling)
...
- The iMac repeatedly makes **outbound HTTPS** requests to Telegram's Bot API:
...
**Important:** the iMac initiates every connection. Telegram never connects *into* your home network.

---

### Step 3: AI runs locally (no cloud inference)

Once a message arrives, processing stays on the iMac:
...
| Direction | Works? | Why |
|-----------|--------|-----|
| **Outbound** (iMac → internet) | Yes | Router rewrites source IP and tracks the session |
| **Inbound** (internet → iMac) | No (by default) | No one on the internet knows your private `192.168.x.x` |

Your bot only needs **outbound HTTPS (TCP port 443)**. Home routers allow that by default.

You do **not** need:

- A public IP on the iMac
- Port forwarding (80, 443, etc. to the iMac)
- Dynamic DNS
...
Link:         Wi‑Fi or Ethernet on the iMac
```
...
## What Must Be Working on the iMac

### For Telegram messaging

1. **Internet access** — DNS resolution + outbound TCP 443
2. **Valid bot token** — authenticates your iMac to Telegram's API
...
| Mode | How it works | Home iMac fit |
|------|----------------|---------------|
| **Long polling** (this project) | iMac pulls updates from Telegram | Ideal — outbound HTTPS only |
| **Webhook** | Telegram POSTs updates to your public URL | Requires public HTTPS endpoint + open port or tunnel |

This project uses **long polling** (`app.run_polling()`). Webhooks would require exposing the iMac to the internet, which is harder to set up and less appropriate for a home machine.
...
Because the iMac only makes **outbound** calls to Telegram:
...
2. iMac: GET/POST api.telegram.org/getUpdates (long poll)
      ← Telegram returns your message JSON

3. iMac: POST 127.0.0.1:11434/api/chat (Ollama)
      ← model generates answer locally

4. iMac: POST api.telegram.org/sendMessage
      → Telegram cloud
...
**Parts stay entirely local:** Ollama inference and model weights on the iMac.
